...
def assign_speakers(
    ...
    Picks the diarization interval with the largest temporal overlap.
    Falls back to ``"SPEAKER_00"`` when *diarization* is empty. Input
    *segments* is not mutated; a new list of new dicts is returned.
    """
    labeled: list[dict] = []
    for seg in segments:
        new_seg = dict(seg)
